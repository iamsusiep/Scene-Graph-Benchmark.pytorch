...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        #print("relationhead, roi_features,", roi_features)
        orig_features = None#orig_features = roi_features.clone().detach()
        ...
        if not self.training:
            postout= self.post_processor((relation_logits, refine_logits), rel_pair_idxs, proposals)
            #print("postout", postout)
            result = postout
            return roi_features, result, {}#, orig_features, proposals
